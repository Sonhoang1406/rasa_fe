...
import { CreateStoryRequest, StepStory } from "@/lib/types/stories-type";
...
interface CreateStoriesDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onStoryCreated: () => void;
}

export function CreateStoriesDialog({
  open,
  onOpenChange,
  onStoryCreated,
}: CreateStoriesDialogProps) {
  ...
  const form = useForm<{
    ...
    resolver: zodResolver(CreateStoriesSchema),
    defaultValues: {
      name: "",
      desc: "",
      steps: [{ type: "INTENT" }],
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-[90vw] sm:max-w-lg max-h-[80vh] overflow-y-auto p-4">
        <DialogHeader>
          <DialogTitle>{t("Create Story")}</DialogTitle>
          <DialogDescription>
            {t("Enter details for the new story.")}
          </DialogDescription>
        </DialogHeader>
        <Form {...form}>
