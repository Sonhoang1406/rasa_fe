...
export function EditRuleDialog({
  ...
  const form = useForm<{
    ...
    resolver: zodResolver(editRuleSchema) as any,
    defaultValues: {
      name: "",
      desc: "",
      intent: "",
  ...
  const onSubmit = async (data: {
    ...
    if (!rule) return;
    try {
      const ruleData: UpdateRuleRequest = {
        name: data.name,
        desc: data.desc,
        steps: [
          {
      ...
      await updateRule(rule._id, ruleData);
      onRuleUpdated();
      onOpenChange(false);
      form.reset();
    } catch (error) {
