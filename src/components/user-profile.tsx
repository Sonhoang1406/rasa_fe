    ...
    const fetchUserProfile = async () => {
      try {
        ...
        reset({
          ...
          dob: userProfile.dob.substring(0, 10),
    ...
    <div className="p-4 md:p-6 max-w-4xl w-full mx-auto min-h-screen flex flex-col gap-6">
      <div className="bg-gradient-to-r from-blue-100 to-yellow-100 rounded-xl p-4 md:p-6 transition-all duration-300 ease-in-out dark:from-blue-800 dark:to-yellow-800">
        ...
        <div className="flex items-center space-x-4 relative">
          ...
          <div>
            <h3 className="text-lg md:text-xl font-semibold text-gray-900 dark:text-white">{user.username}</h3>
            <p className="text-sm md:text-base text-muted-foreground dark:text-gray-300">{user.email}</p>
          </div>
      ...
      >
        <div>
          ...
          {isEditing ? (
            <Input {...register("name")} placeholder="Your First Name" className="text-sm mt-1 md:text-base dark:bg-gray-700 dark:text-white" />
          ) : (
        ...
        <div>
          <label className="text-sm md:text-base font-medium">Date of Birth</label>
          {isEditing ? (
            <Popover>
                ...
                <Calendar
                  ...
                  onSelect={(date) => {
                    if (date) {
                      ...
                      localDate.setMinutes(localDate.getMinutes() - localDate.getTimezoneOffset()); // Điều chỉnh theo múi giờ
            ...
            <p className="mt-1">{new Date(user.dob).toLocaleDateString("vi-VN")}</p>
          )}
        </div>
        ...
        <div>
          ...
          {isEditing ? (
            <Input {...register("address")} placeholder="Your Address" className="text-sm mt-1 md:text-base dark:bg-gray-700 dark:text-white" />
          ) : (
        ...
        <div>
          ...
          {isEditing ? (
            <Input {...register("phone")} placeholder="Your Phone" className="text-sm mt-1 md:text-base dark:bg-gray-700 dark:text-white" />
          ) : (
        ...
        <div className="col-span-2">
          <label className="text-sm md:text-base font-medium">My email Address</label>
            ...
            <Button
              onClick={async () => {
                ...
                try {
                  ...
                  setPasswordError("Đổi mật khẩu không thành công, thử lại sau");
                }
              }}
            >
              Lưu
      ...
      <Dialog open={openAvatarDialog} onOpenChange={setOpenAvatarDialog}>
        <DialogContent className="bg-card text-card-foreground">
          ...
          <DialogFooter className="mt-4 flex justify-end gap-2">
            <Button variant="outline" onClick={() => setOpenAvatarDialog(false)}>
              Hủy
            </Button>
            <Button
              onClick={async () => {
